...
def start_monitoring(path, start_button):
    ...
    start_button.configure(text="Обход Включён\Не закрывайте программу", fg_color="green", state="disabled")
...
def open_rust():
    ...
    if os.path.exists(path_file):
        ...
        rust_folder = ctk.filedialog.askdirectory(title="Выберите папку с растом")
        app.destroy()
        if not rust_folder:
            return ""
...
def inject_other():
    app = ctk.CTk()
    app.attributes('-topmost', True)
    dll_path = ctk.filedialog.askopenfilename(title="Выберите DLL файл для инжекта", filetypes=[("DLL Files", "*.dll")])
    app.destroy()
    if not dll_path:
...
def main():
    ...
    title_text = "Обход Alkad EAC by usehvh :3"
    animate_title(root, title_text)

    main_frame = ctk.CTkFrame(root)
    main_frame.pack(pady=20, padx=10)

    start_button = ctk.CTkButton(main_frame, text="Включить Обход", command=lambda: start_monitoring(load_path() or open_rust(), start_button))
    ...
    open_rust_button = ctk.CTkButton(main_frame, text="Открыть Rust", command=open_rust)
    open_rust_button.grid(row=2, column=0, padx=10, pady=5, sticky="ew")

    close_rust_button = ctk.CTkButton(main_frame, text="Закрыть Rust", command=close_rust)
